...
### 웹소켓 서버 구현 <a href="https://github.com/Mason3144/seb44_main_019/tree/main/backend/websocket" target='_blank'> [프로젝트 링크] </a>
...
1. Spring Security 프레임워크를 설치하여 사용자 인증방식을 메세지 전송시가 아닌 처음 클라이언트와 서버의 웹소켓 프로토콜이 연결되는 시점으로 변경
...
특정 채팅방의 이전 메세지들을 불러오기 위해서 발신되는 모든 메세지들을 DB에 저장해야만 했고 
실시간 채팅 특성상, 처리속도가 중요하다 생각하여 멀티 쓰레드를 이용하게 되었습니다.
...
### 환경변수 관리
이번 ShellWe 프로젝트 이전, AWS EC2 프리티어 버전의 Ubuntu 운영체제에 환경변수를 직접 저장하여 사용한 경험이 있었습니다. 
...
### 예외 처리
**@RestControllerAdvice** 어노테이션을 사용하여 발생될 가능성이 있는 다양한 예외들을 한곳에 모아 처리하였습니다.
다양한 예외 코드들 또한 Enum 타입으로 지정하여 유지보수성을 높였습니다.
