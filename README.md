# ShellWe 프로젝트 포트폴리오 목차
1. [프로젝트 개요](#-프로젝트-개요)
2. [Member](#-member)
3. [STACKS AND TOOLS](#-stacks-and-tools)
4. [어플리케이션 설계도](#-어플리케이션-설계도)
5. [테이블 ERD](#-테이블-erd)
6. [주요 기능 요약](#-주요-기능-요약)
7. [내가 맡은 기능](#-내가-맡은-기능)
8. [릴리즈 이후 보완 상황](#-릴리즈-이후-보완-상황)
9. [문제 발생 및 해결](#-문제-발생-및-해결)
10. [개발 당시 고려 사항](#-개발-당시-고려-사항)

# 👨‍💻 프로젝트 개요

약 5주간 진행된 팀 프로젝트로 기획부터 설계, 개발, 배포까지 진행했습니다. </br>
저희 팀에서는 다른 팀들과 차별화를 두기위해 구글 SMTP서버를 이용하여 회원가입 시 인증코드를 사용자에게 발송하고 WebScoekt을 이용한 실시간 채팅기능을 구현했습니다. 
...
사용자는 자유롭게 자신의 제품을 올리고 다른사용자의 제품과 매칭한 뒤 DM 기능을 이용하여 자유롭게 교류합니다.

### 배포 주소 : http://shellwe.net/
...
팀원들의 의견으로 프로젝트 종료 후에도 몇달간 서버를 열어둘 것을 결정하게 되었습니다. <br>
그에 따라 비용적인 측면을 고려하여 어플리케이션 설계를 다시 진행하여 적용하게되었습니다.<br>

현재 적용된 어플리케이션 설계는 다음과 같습니다.
1. 하나의 S3 인스턴스만 개설하여 3개의 버킷으로 나눈 뒤 클라이언트, 이미지 파일, 배포 목적으로 사용
2. 하나의 EC2 인스턴스만 개설하여 백엔드와 웹소켓 서버를 가동
3. AWS-Elastic Load Balancer와 RDS 개설을 생략, DB의 경우 MySql을 EC2 인스턴스에 설치하여 연동

위와 같이 적용하여 월간 2만원대의 예상 지출비용을 2천원대로 약 1/10로 감소시켰습니다.
(DNS 가격, 연간 $11 제외)
...
현재 틈틈이 어플리케이션 설계에 대해 공부중입니다. <br>
캐시서버, CDN, 다중 DB서버등을 적용하여 더 큰 규모의 설계를 구상해보고 싶은 바램이 있습니다.
...
다양한 예외 코드들을 Enum 타입으로 지정하고 예외 발생시의 response body 형태를 통일하여 예외처리의 유연성을 높였습니다.
